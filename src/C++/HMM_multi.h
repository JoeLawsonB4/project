...
public:
	...
	*/
	HMM_multi(VEC2D(double) m_P, VEC2D(double) m_E, double m_tol, int m_maxIt, VEC(int) m_params, string m_filename = ""):\
	...
	*/
    ...
    void HMMGameSample(VEC(double) nu, VEC2D(double) P, VEC2D(double) E, int * x, int * y){
        int k;
        // Sample first transition
        // (*x)[0] = randm(nu);
        // (*y)[0] = randm(E[(*x)[0]]);
	...
	*/
	double HMMfilter(int * y, double ** P_e, double ** E_e, double ** fs,  double * c){
		int i,j,t;
		double ll = 0;
	...
	double HMMfilter_opt(int * y, double ** P_e, double ** E_e, double ** fs,  double * c, int * nzc, int ** nzc_row, int opt){
		int i,j,k,t;
		double ll = 0;
		VEC(double) alpha(N,0.0);
		// set fs to 0
	...
	*/
	double HMMtrain(int ** y, double ** P_est, double ** E_est, double tol, int maxIt){
		int i, j, r, it, t;
		// int k;
		r = 0;
		double z = 0;
		...
		for(it=0; (change > tol) && (it<maxIt); it++){
			...
			for(r = 0; r < seq; r++){
				...
				ll = HMMfilter(y[r], P_tmp, E_tmp, fs, c);
	...
	out:  P = estimate of the transition matrix of the hidden markov process
		  ...
		  l = log-likelihood of y for parameters P and E
	*/
	double HMMtrain_opt(int ** y, double ** P_est, double ** E_est, double tol, int maxIt){
		...
		*/
		...
		for(it=0; (change > tol) && (it<maxIt); it++){
			...
			if(change < 1e-3 || opt > 0){
				...
				for(r = 0; r < seq; r++){
					ll = HMMfilter_opt(y[r], P_tmp, E_tmp, fs, c, nzc, nzc_row, opt);
	...
	*/
	double run(double ** P_e, double ** E_e, int ** y, int uniform_guess = 0, int mode = 0){
		int i,j;
		...
		begin = clock();
		ll = HMMtrain_opt(y, P_est, E_est, tol, maxIt);
		end = clock();
